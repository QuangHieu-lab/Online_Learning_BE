...
function buildLessonProgressState({
  ...
  const passedQuiz = Boolean(
    quiz &&
      (quizAttempts || []).some((attempt) => Number(attempt.totalScore || 0) >= Number(quiz.passingScore || 0))
  );
  ...
  const hasAnyActivity =
    normalizeProgressStatus(learningProgress?.status) === PROGRESS_STATUS_IN_PROGRESS ||
    normalizeProgressStatus(learningProgress?.status) === PROGRESS_STATUS_COMPLETED ||
...
function buildEnrollmentProgressSnapshot({
  ...
  const resourceProgressMap = new Map(
    (resourceProgressRecords || []).map((record) => [record.resourceId, record])
  );
  ...
  const modules = orderedModules.map((module) => {
    ...
    const completedLessons = lessons.filter((lesson) => lesson.status === PROGRESS_STATUS_COMPLETED).length;
    const moduleStatus =
      lessons.length > 0 && completedLessons === lessons.length
        ? PROGRESS_STATUS_COMPLETED
        : lessons.some((lesson) => lesson.status !== PROGRESS_STATUS_NOT_STARTED)
          ? PROGRESS_STATUS_IN_PROGRESS
          : PROGRESS_STATUS_NOT_STARTED;
  ...
  const completedLessons = flatLessons.filter((lesson) => lesson.status === PROGRESS_STATUS_COMPLETED).length;
  const totalLessons = flatLessons.length;
  const percentage = totalLessons > 0 ? Math.round((completedLessons / totalLessons) * 100) : 0;
  const currentLesson =
    flatLessons.find((lesson) => lesson.status !== PROGRESS_STATUS_COMPLETED) || null;

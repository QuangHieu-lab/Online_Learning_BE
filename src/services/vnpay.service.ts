...
const VNPAY_RETURN_URL = process.env.VNP_RETURN_URL || process.env.VNPAY_RETURN_URL || 'http://localhost:3000/api/payments/vnpay-return';

export interface VNPayPaymentData {
  amount: number; // Amount in VND
  orderId: string; // Order ID (unique)
 ...
 */
export const createVNPayPaymentUrl = (paymentData: VNPayPaymentData): string => {
  const {
    amount,
    orderId,
    orderDescription,
 ...
 */
export const verifyVNPayCallback = (queryParams: Record<string, string>): {
  isValid: boolean;
  data: {
    vnp_TxnRef: string;
    vnp_Amount: number;
